...
macro_rules! build_entry {
   ($starter:path, $entry:ident, $osapi:path, $($proc:literal),*)  => {
      ...
      extern "system" fn DllMain(
         handle_dll  : __nusion_osapi::shared::minwindef::HINSTANCE,
         ...
         let handle_thread = unsafe{__nusion_osapi::um::processthreadsapi::CreateThread(
            0 as __nusion_osapi::um::minwinbase::LPSECURITY_ATTRIBUTES,
            0,
            Some(__nusion_slib_main_thread),
            handle_dll as __nusion_osapi::shared::minwindef::LPVOID,
            0,
            0 as __nusion_osapi::shared::minwindef::LPDWORD,
         )};
         if handle_thread == 0 as __nusion_osapi::shared::ntdef::HANDLE {
            if unsafe{__nusion_osapi::um::libloaderapi::FreeLibrary(
               handle_dll as __nusion_osapi::shared::minwindef::HMODULE,
            )} == __nusion_osapi::shared::minwindef::FALSE {
               let err = unsafe{__nusion_osapi::um::errhandlingapi::GetLastError()};
      ...
      extern "system" fn __nusion_slib_main_thread(
         handle_dll : __nusion_osapi::shared::minwindef::LPVOID,
